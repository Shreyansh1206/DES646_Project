...
Upload a workout video, run MediaPipe + PyTorch inference, and stream live feedback via a Django backend with a modern Vite + React frontend. Optional TTS feedback (text-to-speech) is supported. This repository contains the model code plus a minimal web UI layer.
...
- Streaming is implemented with Server-Sent Events (SSE); you can tune thresholds and timings in `webapp/webapp/inference/services.py`.
- The React frontend provides live feedback (angles, scores, textual coaching) during processing.
- If you encounter MediaPipe import errors, ensure Python 3.10 or 3.11.
