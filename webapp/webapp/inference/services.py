...
def stream_feedback_for_video(video_path: str) -> Generator[str, None, None]:
    ...
    try:
        ...
        if predicted and required_angles:
            # Exercise-specific scorers and reference keys
            ...
            # Build input dictionary for NLP feedback
            joint_names = [
            ...
            # Generate NLP feedback via Groq API
            import json as _json
            import ast
            text_feedback = feedback_fn(input_dic).strip()
            # Try strict JSON first, then Python literal, then fallback
            try:
